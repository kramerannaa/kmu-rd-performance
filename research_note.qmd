...
# NOTE: adjust this path if your cleaned panel lives elsewhere
df = pd.read_parquet("data/processed/panel_clean.parquet")

# Data quality filter: drop firm-years with non-positive total assets
# (at <= 0 or missing breaks every ratio built on at, and log(at) is undefined)
df = df[df["at"] > 0].copy()

# Derived variables (raw Compustat fields -> research variables)
df.loc[:, "roa"] = df["nicon"] / df["at"]
df.loc[:, "rd_intensity"] = df["xrd"] / df["at"]
df.loc[:, "ln_at"] = np.log(df["at"])
df.loc[:, "capx_intensity"] = df["capx"] / df["at"]
df.loc[:, "cash_ratio"] = df["che"] / df["at"]

# Leverage requires seq > 0 (negative/zero equity makes the ratio meaningless)
df = df[df["seq"] > 0].copy()
df.loc[:, "leverage"] = (df["dltt"] + df["dlc"]) / df["seq"]

# Drop any remaining rows with missing values in core variables
...
rd_share = round((df["xrd"] / df["at"] > 0).mean() * 100, 1)
...
The empirical analysis draws on annual firm-level data from the WRDS Compustat Global database (`comp_global_daily.g_funda`). The sample is restricted to EUR-reporting firms satisfying the EU SME definition (at most 250 employees, annual revenue at most EUR 50 million, or total assets at most EUR 43 million) over fiscal years 2015–2024.

We apply standard data quality filters — requiring strictly positive total assets and positive shareholder equity, since both enter the denominator of core ratios — and require at least three consecutive firm-year observations per firm. The resulting unbalanced panel comprises `{python} f"{n_obs:,}"` firm-years from `{python} f"{n_firms:,}"` unique firms.
...
desc = pd.DataFrame({
    ...
    "Leverage (dltt+dlc/seq)": df["leverage"],
