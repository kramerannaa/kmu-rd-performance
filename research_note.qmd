...
We apply standard data quality filters and require at least three consecutive firm-year observations per firm. The resulting unbalanced panel comprises `{python} f"{n_obs:,}"` firm-years from `{python} f"{n_firms:,}"` unique firms.
...
```{python}
#| label: fig-rd-roa
#| fig-cap: "R&D intensity versus RoA for R&D-active firms (left) and median RoA by firm size for firms with and without R&D expenditure (right)."

# TODO: paste your plotting code from 03_descriptives.py here
```

```{python}
#| label: fig-corr
#| fig-cap: "Pairwise correlation matrix of the research variables."

# TODO: paste your correlation heatmap code from 03_descriptives.py here
```
