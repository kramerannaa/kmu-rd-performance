...
Hypotheses
----------
H0: R&D Intensität hat keinen oder einen positiven Effekt auf RoA.
H1: β(rd_intensity) < 0 and significant
    Höhere R&D Intensität führt zu niedrigerer RoA bei europäischen KMU
    (expensing effect: R&D Ausgaben reduzieren kurzfristig den Nettogewinn).
H2: β(rd_x_size) > 0
    Firmengröße moderiert den negativen Zusammenhang positiv —
    bei größeren KMU ist der negative Effekt schwächer.
...
print(f"  {'Prefer FE (non-trivial difference)' if diff > 0.005 else 'Small difference — FE still recommended'}")

# ── Results table ─────────────────────────────────────────────────────────────
print("\n" + "="*65)
print("Regression Results — Innovationsintensität & Firmenperformance KMU")
print("="*65)
...
if p_x < 0.10:
    if b_x < 0:
        print("  H1 SUPPORTED: negativer Effekt wie vorhergesagt")
        print("  R&D Ausgaben reduzieren kurzfristig RoA (expensing effect)")
        print("  H0 wird abgelehnt")
    else:
        print("  H1 NOT SUPPORTED: Effekt ist positiv (unerwartete Richtung)")
        print("  H0 kann nicht abgelehnt werden")
else:
...
if p_int < 0.10:
    if b_int > 0:
        print("  H2 SUPPORTED: größere KMU profitieren mehr von R&D")
        print("  Konsistent mit Absorptive Capacity Theorie (Cohen & Levinthal 1990)")
    else:
        print("  H2 NOT SUPPORTED: negativer Moderationseffekt")
        print("  Kleinere KMU könnten stärker von R&D profitieren")
else:
...
if pct_diff > 20:
    print("  Großer Unterschied → Omitted Variable Bias in OLS")
    print("  Firm FE kontrolliert für zeitinvariante Heterogenität")
